  ...
  componentDidMount = (props) => {
    ...
    fetch(request)
      ...
      .then((resp) => {
        ...
        this.setState({
          ratingCount: `${resp.data.length}`,
          reviewCount: `${resp.data.length}`,
          averageRatingCount: `${aValue / resp.data.length}`,
    ...
    return (
      <>
        ...
        <Modal className="rating" isOpen={this.state.loginFormOpen}>
          <div className="ratingContainer">
            <div className="ratingClosebuttonContainer">
              <button className="ratingClosebutton" onClick={this.hideLogin}>
                X
              </button>
            </div>
            ...
            <ul className="rateBtnContainer">
              ...
              <li>
                <button onClick={this.submitReviews}>Submit</button>
              </li>
            </ul>
            <button onClick={this.removeRating} className="remoteRating">
              Remove Response
            </button>
          </div>
        </Modal>
        <div className="date">
          ...
          <label className="spaceBetweenText">{this.state.ratingCount} </label>
          Review Count - {this.state.reviewCount}
          <label className="spaceBetweenText"> Average</label> Rating Count -{" "}
          {this.state.averageRatingCount}
        </div>
        <span onClick={this.openReviews} className="reviewOverviewText">
          Reviews{" "}
          {[...Array(10)].map(() => {
            return <FaStar color="yellow" size={13} />;
          })}
        </span>
        <>
                  ...
                  .map((index) => {
                    ...
                    return (
                      <>
                        ...
                        <ul className="ol">
                          Review - {this.state.review[value]}
                        </ul>
                        <ul className="ol">
                          Rating - {this.state.rate[value]}
                        </ul>
                      </>
                    );
                  })}
